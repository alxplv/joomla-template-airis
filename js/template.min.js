...
function CreateDoubleGisWidgetMap(containerId, mapLatitude, mapLongitude, mapOrganizationId, mapCity, mapBorderColor, mapWidth, mapHeight, mapZoomLevel) {
	...
	if (typeof mapBorderColor !== "string") {
		mapBorderColor = "#a3a3a3";
	}

	if (typeof mapWidth !== "string") {
		mapWidth = "100%";
	}

	if (typeof mapHeight !== "number" || mapZoomLevel < 1) {
		mapHeight = 400;
	}

	if (typeof mapZoomLevel !== "number" || mapZoomLevel < 0 || delay % 1 !== 0) {
		mapZoomLevel = 16;
	}
...
function LoadLazyStylesheets() {
	document.head.querySelectorAll("[rel=\"lazy-stylesheet\"]").forEach(function (element) {
...
function CreateBootstrapToast(content = "", header = "", delay = 5000, autohide = true, animation = true) { // BS defaults
	...
	if (bootstrap === "undefined") {
		console.log("Bootstrap API not found.");
		return;
	}

	if (!Joomla.getOptions("tpl_airis").useBootstrapJsComponentToasts) {
		console.log("Bootstrap Toasts component not found.");
		return;
	}

	if ((typeof content !== "number" && typeof content !== "string") || content === "") {
		console.log("No toast content provided.");
		return;
	}

	if (typeof delay !== "number" || delay < 0 || delay % 1 !== 0) { // Make sure the delay is not negative or a float
		delay = 5000;
	}
	...
	if (!toastContainer) {
		console.log("Bootstrap Toasts container not found in DOM.");
		return;
	}
