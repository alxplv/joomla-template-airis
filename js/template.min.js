...
function CreateDoubleGisWidgetMap(containerId, mapLatitude, mapLongitude, mapOrganizationId, mapCity, mapBorderColor, mapWidth, mapHeight, mapZoomLevel) {
	...
	// This data attribute is used to detect whether the map for this organization has already been created in the container
	if (mapContainer.dataset.doublegisMapOrganizationId === mapOrganizationId) {
		return;
	} else {
		mapContainer.dataset.doublegisMapOrganizationId = mapOrganizationId;
		// Flush the container if new organization map is being created
...
function LoadExtremelyLazyIframes() {
	document.body.querySelectorAll("[data-extra-lazy-iframe-src]").forEach((element) => {
		element.src = element.dataset.extraLazyIframeSrc;
	});
}
