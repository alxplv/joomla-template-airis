...
<div id="cart-view" class="cart-view">
	...
	?><form method="post" id="checkoutForm" name="checkoutForm" action="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=cart' . $taskRoute, $this->useXHTML, $this->useSSL); ?>">
		...
		// Use Joomla!'s reCAPTCHA plugin during order confirmation step for guest users

		// TODO: Remove Joomla 3 code
		// TODO: reCaptcha field disappears during any ajax update of the cart like changing the product quantity or shipment/payment options so disabling the "Ajax for OPC" VirtueMart option is the only fix for that now

		// Joomla! imports
		// use Joomla\Event\Event; // Joomla! 4
		use Joomla\CMS\Factory;
		use Joomla\CMS\Plugin\PluginHelper;

		if (($this->checkout_task == 'confirm') && Factory::getUser()->guest && (Factory::getConfig()->get('captcha', false) == 'recaptcha'))
		{
			PluginHelper::importPlugin('captcha');
			$joomlaApplication = Factory::getApplication();

			// Prepare HTML container class and unique id attribute values for the captcha plugin
			$captchaContainerClass = 'virtuemart-cart-captcha-order-confirm';
			$captchaContainerIdSalt = md5(time());
			$captchaContainerId = "$captchaContainerClass-$captchaContainerIdSalt";

			// Prepare plugin events (Joomla! 4)
			// $joomlaEventDispatcher = $joomlaApplication->getDispatcher();
			// $joomlaCaptchaPluginEventOnInit = new Event('onInit', $captchaContainerId);
			// $joomlaCaptchaPluginEventOnDisplay = new Event('onDisplay', array(null, $captchaContainerId, $captchaContainerClass));

			// Joomla! 3
			$joomlaEventDispatcher = JEventDispatcher::getInstance();

			// The onInit() event handler of the captcha plugin can throw exceptions
			try
			{
				// $joomlaEventDispatcher->dispatch('onInit', $joomlaCaptchaPluginEventOnInit); // Joomla! 4
				$joomlaEventDispatcher->trigger('onInit', $captchaContainerId); // Joomla! 3
			}
			catch (Exception $joomlaCaptchaException)
			{
				$joomlaApplication->enqueueMessage($joomlaCaptchaException->getMessage(), 'error');
			}

			// Get the captcha plugin output
			// $joomlaCaptchaHtml = implode($joomlaEventDispatcher->dispatch('onDisplay', $joomlaCaptchaPluginEventOnDisplay)); // Joomla 4
			$joomlaCaptchaHtml = implode($joomlaEventDispatcher->trigger('onDisplay', array(null, $captchaContainerId, $captchaContainerClass)));
			echo $joomlaCaptchaHtml;
		}
