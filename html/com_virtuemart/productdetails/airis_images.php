...
$productImageClasses = 'productdetails-image-file productdetails-image-file-main';
...
if (empty($productImage->file_meta)) $productImage->file_meta = $productName;
...
if ($templateParameters->get('loadFancybox'))
{
	...
	$productImageLinkAttributes .= (count($this->product->images) == 1 || VmConfig::get('add_img_main', 0)) ? ' data-fancybox' : ' data-fancybox="productdetails-gallery-item"';
	$productImageLightboxCaption = '';

	// Try using the alt value if there was no subtitle defined for this media
	if (!empty($productImage->file_description))
	{
		$productImageLightboxCaption = $productImage->file_description;
	}
	elseif ($productImage->file_meta !== $productName)
	{
		$productImageLightboxCaption = $productImage->file_meta;
	}

	if (!empty($productImageLightboxCaption)) $productImageLinkAttributes .= " data-caption=\"$productImageLightboxCaption\"";

}
/* elseif ($templateParameters->get('loadGlightbox'))
{
	vmJsApi::addJScript('productdetails-gallery-glightbox', '
...
{
	// Explicitly set thumbnail width and height here since this is what we do in displayMediaThumb() below and otherwise getFileUrlThumb() returns a thumbnail generated at media creation time sized in accordance with a different set of VM settings
	$productImageJoomlaImageClassInstance = new Image($productImage->getFileUrlThumb($thumbnailWidthConfigurationValue, $thumbnailHeightConfigurationValue));
	echo $productImage->displayMediaThumb(array('class' => $productImageClasses, 'width' => $productImageJoomlaImageClassInstance->getWidth(), 'height' => $productImageJoomlaImageClassInstance->getHeight()), true, $productImageLinkAttributes, true, false, false, $thumbnailWidthConfigurationValue, $thumbnailHeightConfigurationValue);
}
else
{
	$productImageJoomlaImageClassInstance = new Image($productImage->getUrl());
	echo $productImage->displayMediaFull(array('class' => $productImageClasses, 'width' => $productImageJoomlaImageClassInstance->getWidth(), 'height' => $productImageJoomlaImageClassInstance->getHeight()), true, $productImageLinkAttributes, false);
}